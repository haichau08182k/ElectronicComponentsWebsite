...
namespace electronicComponents.Repository
{
    public class GenericRepository<Tbl_Entity> : IRepository<Tbl_Entity> where Tbl_Entity : class
    {
        DbSet<Tbl_Entity> _dbSet;

        private electronicComponents1Entities1 _DBEntity;

        public GenericRepository(electronicComponents1Entities1 DBEntity)
        {
            _DBEntity = DBEntity;
        ...
        {
            return (IEnumerable<Tbl_Entity>)_dbSet.ToList();
        }
        ...
        public IEnumerable<Tbl_Entity> GetAllRecords(Expression<Func<Tbl_Entity, bool>> where)
        {
            return (IEnumerable<Tbl_Entity>)_dbSet.Where<Tbl_Entity>(where).ToList();
        }
